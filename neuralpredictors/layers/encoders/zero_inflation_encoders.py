...
class MLP(nn.Module):
    def __init__(self, input_size, layer_sizes):
        ...
        for out_channels in layer_sizes:
            layers.append(nn.Linear(in_channels, out_channels,))  # Add a linear layer
...
class FlowLayer(nn.Module):
    ...
    def forward(self, z, zero_mask):
        ...
        log_det_soft = -torch.log(torch.sigmoid(self.invert_softplus(z))) #sigmoid is derivative of sofplus f'^-1 = 1/f'(f())
    ...
    def invert_elu(self, y):
        ...
        return torch.where(
            y > 0, y, torch.log((y / self.elu1.alpha + 1).clamp(min=finfo.tiny))
        )
...
class Response_Encoder(nn.Module):
    def __init__(self, input_dim, hidden_dim, mice_dim, hidden_gru, output_dim, n_neurons, n_hidden_layers=3, norm="batch",
                     non_linearity=True, use_cnn=False, cnn_kernel_sizes=None, cnn_hidden_channels=None,use_resnet=False, 
                     pre_trained=False,residual=False, **kwargs):
        super().__init__()
...
class Response_Decoder(nn.Module):
    def __init__(self, input_dim, hidden_dim, n_layers=2, use_cnn=False, kernel_sizes=None, hidden_channels=None,):
...
class ZIGEncoder(ZeroInflationEncoderBase):
    ...
    # behavior_in_encoder: If it's a dict, contains dict of MLP which processes behavior to add it to responses input in encoder
        ...
        if latent:
            ...
            if flow:
                ...
                self.psi = {
                    data_key: torch.ones(n_neurons[data_key])
                    for data_key, _ in self.readout.items()
                }
            else:
                self.flow = None
            ...
            self.encoder = Response_Encoder(encoder["input_dim"],encoder["hidden_dim"],encoder["mice_dim"],encoder["hidden_gru"],encoder["output_dim"],
                                            ...
                                            use_cnn=encoder["use_cnn"], cnn_kernel_sizes=encoder["kernel_size"], cnn_hidden_channels=encoder["channel_size"],
                                            use_resnet = encoder["use_resnet"],pre_trained=encoder["pretrained"],residual=encoder["residual"],behavior_in_encoder = behavior_in_encoder)
            ...
            if decoder:
                ...
                self.decoder = Response_Decoder(input_dim=encoder["output_dim"]+self.output_dim_behavior, hidden_dim = decoder["hidden_dim"],n_layers=decoder["hidden_layers"], use_cnn=decoder["use_cnn"], #if Behavior is added to the latent, adapt the latent dim
                                             kernel_sizes=decoder["kernel_size"], hidden_channels=decoder["channel_size"])
            ...
            if position_features:
                ...
                self.mlp_q = nn.ModuleDict((datakey, MLP(position_features["input_size"],position_features["layer_sizes"])) for datakey in n_neurons)
                #self.mlp_theta = MLP(position_features["input_size"],position_features["layer_sizes"])
                self.mlp_theta = nn.ModuleDict((datakey, MLP(position_features["input_size"],position_features["layer_sizes"])) for datakey in n_neurons)
                self.latent_feature = {}
                ...
                '''
                self.v1_vectors = nn.ParameterDict({
                    data_key + suffix + "_v1": nn.Parameter(torch.randn(out_dim))  # First vector of size out_dim
                    for data_key, ro in readout.items()
                    for suffix in ["q", "theta"]
                })
                
                self.v2_vectors = nn.ParameterDict({
                    data_key + suffix + "_v2": nn.Parameter(torch.randn(ro.outdims))  # Second vector of size ro.outdims
                    for data_key, ro in readout.items()
                    for suffix in ["q", "theta"]
                })
    ...
    def forward(
        self, x, data_key, behavior_core=None, pupil_center_core=None, trial_idx=None, shift=None, detach_core=False, out_predicts = True, train = False,
        sample_prior = False,neuron_mask = None,positions = None,**kwargs
    ):
    # latent_params is a tuple containing tensor with means, variance factor and number of samples for mc sampling
        ...
        if "behavior" in kwargs.keys(): #rename behavior, pupil_center keys, which are for latent input, in kwargs to avoid conflicts with core input
        ...
        if self.latent:
            ...
            if self.behavior_in_encoder:
                ...
                behavior = self.mlp_behavior[data_key](behavior) # apply MLP to scale dim from 2/4 to bigger dim like 10
            ...
            if self.dropout:
                if train:
                    ...
                    responses = 1/(1-self.dropout_prob) * responses #after dropout non zero values are multiplied with 1/(1-p), p=0.5 here
                ...
                means = self.encoder(kwargs["responses"],data_key, self.mice[data_key][0:total_time,:],behavior=behavior)
            ...
            if (not out_predicts) or sample_prior:
                ...
                if self.position_features:
                    latent_feature_theta = self.mlp_theta[data_key](positions).permute(1,0) #shape to (latent_dim, n_neurons)
                    latent_feature_q = self.mlp_q[data_key](positions).permute(1,0) 

                    latent_repr_theta =  torch.einsum('bhi,hj->bji', samples, latent_feature_theta) #this computes w_z*z and has shape (B*time,n_neurons,samples)
                    latent_repr_q =  torch.einsum('bhi,hj->bji', samples, latent_feature_q) #this computes w_z*z and has shape (B*time,n_neurons,samples)
                    ...
                    #v1_q = self.v1_vectors[data_key + "q_v1"]
                    #v2_q = self.v2_vectors[data_key + "q_v2"]
                    #latent_feature_q = torch.outer(v1_q, v2_q)

                    #v1_theta = self.v1_vectors[data_key + "theta_v1"]
                    #v2_theta = self.v2_vectors[data_key + "theta_v2"]
                    #latent_feature_theta = torch.outer(v1_theta, v2_theta)
                ...
                if self.position_features:
                    ...
                    latent_repr_theta =  torch.einsum('bh,hj->bj', means, latent_feature_theta) #this computes w_z*z and has shape (B*time,n_neurons,samples)
                    latent_repr_q =  torch.einsum('bh,hj->bj', means, latent_feature_q) #this computes w_z*z and has shape (B*time,n_neurons,samples)
                ...
                logtheta = logtheta + latent_repr_theta #shape logtheta and q to (B*time,n_neurons) and add latent representations
    ...
    def forward_prior(self, x, data_key, behavior_core=None, pupil_center_core=None, trial_idx=None, shift=None, detach_core=False, out_predicts=True,repeats = 10,n_samples=100,positions = None, **kwargs
    ):
        if "behavior" in kwargs.keys(): #rename behavior, pupil_center keys, which are for latent input, in kwargs to avoid conflicts with core input
                kwargs["behavior_encoder"] = kwargs.pop("behavior")
                kwargs["pupil_center_encoder"] = kwargs.pop("pupil_center")
        ...
        for _ in range(repeats):
            if self.latent:
                ...
                logtheta = logtheta_zig.unsqueeze(-1) + latent_repr_theta #shape logtheta and q to (B*time,n_neurons,n_samples) and add latent representations
                q = q_zig.unsqueeze(-1) + latent_repr_q #shape logtheta and q to (B*time,n_neurons,n_samples) and add latent representations
